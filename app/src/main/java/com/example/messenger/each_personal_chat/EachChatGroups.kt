...
import com.example.messenger.EachPersonalMessage
...
class FriendTextChatItem(val text: String, val newPersonalMessage: EachPersonalMessage) :
    ...
    override fun bind(viewHolder: GroupieViewHolder, position: Int) {
        ...
        if (EachPersonalChatFragment.friendUser?.profilePictureUrl != null) {
            Glide.with(viewHolder.itemView.context)
                .load(EachPersonalChatFragment.friendUser?.profilePictureUrl)
                .into(friendImage)
        }
